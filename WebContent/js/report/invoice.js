...
$(document).ready(function () {
	$.post('/bms/bms/get_itemname.do', function(data) {
		$("#itemName").autocomplete({
			 source: data,
			 select: function( event, ui ) {
				 var item_code = ui.item.value;
				 $("#itemCodeStart").textbox('setValue', item_code.substring(0, item_code.indexOf("-")));
			 }
		 });
		$("#itemName").keydown(function(event){
			$("#itemName").val($("#itemName").val().trim());
		});
	}, 'json');
	$.post('/bms/bms/get_supname.do', function(data) {
		$("#supName").autocomplete({
			 source: data,
			 select: function( event, ui ) {
				 var sup_name = ui.item.value;
				 $("#subCode").textbox('setValue', sup_name.substring(0, sup_name.indexOf("-")));
			 }
		 });
		$("#supName").keydown(function(event){
			$("#supName").val($("#supName").val().trim());
		});
	}, 'json');
	initCombobox();
});
...
function getParamsForCombobox() {
	...
	var branch = $('#brCode').combobox('getValue');
